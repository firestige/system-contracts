# Workflow DSL — machine representation（中文翻译）

> **规范语言：英文。** 本文件是 [`README.md`](README.md) 的非规范跟踪翻译；英文变更时整篇重译替换。

本目录是 **Workflow Contract 的 DSL 面**的规范机器表示（`agentops.workflow-dsl@1.0.0`，REVIEW_CANDIDATE）。语义规范文档位于 super project 的 [`docs/contracts/workflow/workflow-definition-dsl.md`](../../docs/contracts/workflow/workflow-definition-dsl.md)（中文翻译：[`workflow-definition-dsl.zh-CN.md`](../../docs/contracts/workflow/workflow-definition-dsl.zh-CN.md)）。
...
- `REVIEW_CANDIDATE`，Contract revision `agentops.workflow-dsl@1.0.0`，按 [Contract Lifecycle Management](../../docs/contracts/contract-lifecycle.md) 管理（快速路径 §4.3）；**未发布**，任何实现不得声称 physical conformance（沿用 `concept.obligation.001` 诚实生命周期约定）。

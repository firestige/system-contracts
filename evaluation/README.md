...
- The checker requires exactly the 14 metric IDs declared by `evaluation.definition`, rejects duplicate metric and input IDs, and rejects missing or unexpected metrics.
- The exact published Observation `1.0.0` semantic revision, machine revision, publication digest and gitlink are closed; the checker resolves the package dependency fail closed.

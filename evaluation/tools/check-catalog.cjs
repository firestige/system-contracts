...
const EXPECTED_INPUT_REFS = {
  "model-role-utility-profile": ["evaluation.unique-terminal-task-outcome", "observation.model-role-attribution-tuple", "observation.model-call-span-duration", "observation.standard-token-usage", "observation.reported-cost", "observation.repair-link", "observation.intervention", "observation.reviewer-finding", "projection.compatibility-eligibility"],
  ...
  "operational-latency-ms": ["observation.model-call-span-duration", "projection.compatibility-eligibility"],
  ...
  "operational-token-usage": ["observation.standard-token-usage", "projection.compatibility-eligibility"],
  "operational-attributable-cost": ["observation.reported-cost", "projection.compatibility-eligibility"],
  "operational-usage-availability": ["observation.model-call-identity", "observation.usage-source", "projection.compatibility-eligibility"],
...
if (!target) {
  ...
  if (catalog && schema) {
    ...
    if (errors.length) {
      ...
      console.log("PASS: 15 metrics, unique IDs, resolved input refs, never-zero missing semantics");
    }
  }
}
